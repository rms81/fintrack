...
public static class TransactionEndpoints
{
    ...
    [WolverineGet("/api/profiles/{profileId}/tags")]
    [Tags("Transactions")]
    [EndpointSummary("Get all unique tags")]
    [EndpointDescription("Returns all unique tags used in transactions for a profile.")]
        ...
        CancellationToken ct)
    {
        ...
        var tags = await db.Transactions
            .Where(t => t.Account!.ProfileId == profileId)
            .SelectMany(t => t.Tags ?? Array.Empty<string>())
            .Distinct()
            .OrderBy(t => t)
            .ToListAsync(ct);
